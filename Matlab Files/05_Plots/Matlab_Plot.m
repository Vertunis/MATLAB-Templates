%% Plot
figure(1);
grid on
hold on
box on
plot(t_plot,R_sim(:,1),':','Color',[0 191 255]/255,'LineWidth',1.5); % Colour in RGB-Farbspektrum angeben
plot(t_plot,R_rec(:,1),'k--','LineWidth',1.5);

xlim_min=0; % Begrenzungen x
xlim_max=30;
ylim_min=0; % Begrenzungen y
ylim_max=1;

xTicks = 5; % Schrittweite x
yTicks = 0.1; % Schrittweite y
set(gca, 'XTickMode', 'manual', 'XTick', xlim_min:xTicks:xlim_max, 'xlim', [xlim_min,xlim_max]); 
set(gca, 'YTickMode', 'manual', 'YTick', ylim_min:yTicks:ylim_max, 'ylim', [ylim_min,ylim_max]);

xlabel('Zeit T [s]'); %Beschriftung x-Achse 
ylabel('Weg x [m]');  %Beschriftung y-Achse
title('Vergleich der Positionsdaten');
legend('Simulation','Messung')