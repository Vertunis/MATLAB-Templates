...
Roboterinfo_pos(:,1:3)=Roboterinfo_pos(:,1:3)/1000;
deltaT_sim=0.01;  %sample Time der Simulation
lR_sim=length(Roboterinfo_pos);
tR_sim=lR_sim*deltaT_sim; % Gesamtdauer in [s]

s=deltaT_sim/deltaT_rec; % s Zeilenverhältnis zwischen R_sim und TCP_pos_m an um der Simulationsdauer 0.02s zu entsprechen
...
t_offset=5.5 % Zeitliches Offset der Messungen in Sekunden
z_offset_sim=t_offset/deltaT_sim;% Umrechnen des zeitlichen Offsets in Zeilennummer

switch 1 % Switch ändern je nach Vektor
    
   case 1 % langer Vektor mit 1751 Einträgen
       t_plot=(0:deltaT_sim:tR_sim-deltaT_sim); % Plotdauer
       R_sim=zeros(lR_sim,6); % Nullvektor erzeugen
       R_rec=zeros(lR_sim,6);
       R_sim=Roboterinfo_pos(1:lR_sim,:); % Vektoren auffüllen
    ...
    case 2 % kurzer Vektor mit 1500 Einträgen
       t_plot=(0:deltaT_sim:tR_rec-deltaT_sim); % Plotdauer
       R_sim=zeros(lR_rec_soll,6);  % Nullvektor erzeugen
       R_rec=zeros(lR_rec_soll,6);
       R_sim=Roboterinfo_pos(1:lR_rec_soll,:); % Vektoren auffüllen
       R_rec(1:lR_rec_soll,1:3)=TCP_pos_m(1:s:lR_rec,:); 
